...
const Layout: React.FC<LayoutProps> = ({ children }) => {
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      {/* Mobile bottom nav — role-adaptive */}
      <nav className="fixed bottom-0 left-0 right-0 z-50 md:hidden bg-card/90 backdrop-blur-xl border-t border-border/50 shadow-[0_-4px_20px_rgba(0,0,0,0.06)]" style={{ paddingBottom: 'env(safe-area-inset-bottom, 0px)' }}>
        <div className="flex justify-around items-center h-16 px-2">
          {navItems.map(({ icon: Icon, label }) => (
            <button
              key={label}
              className="flex flex-col items-center justify-center gap-0.5 py-1.5 px-2 min-w-[56px] text-muted-foreground hover:text-primary active:scale-95 transition-all touch-target rounded-xl"
            >
